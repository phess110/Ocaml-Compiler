...
public class Life {
    ...
    private UI buildUI(RootPaneContainer pane) {
        return new UI(n, pane, pauseIterations, headless, glider, (int)numThreads);
    }
...
class Worker extends Thread {
    ...
    public void run() {
        try {
            ...
            try {
                while (true) {
                    lb.doGeneration(this.start, this.end);
                }
    ...
    // Constructor
    // 
    public Worker(LifeBoard LB, Coordinator C, UI U, int s, int e) { 
        lb = LB;
        c = C;
        u = U;
        start = s;
...
class LifeBoard extends JPanel {
    ...
    private CyclicBarrier barrier; // barrier to prevent threads from leaving doGeneration to soon
    ...
    public void doGeneration(int start, int end) throws Coordinator.KilledException {
        //for (int i = 0; i < n; i++) { use different threads to handle each row(s)
        ...
        try { barrier.await(); //make all threads wait until everyone finishes
    ...
    public LifeBoard(int N, Coordinator C, UI U,
                     boolean hdless, boolean glider, int numThreads) {
        n = N;
        c = C;
        u = U;
        headless = hdless;
        ...
        //initialize barrier
        barrier = new CyclicBarrier(numThreads, new Runnable(){public void run(){T = B; B = A; A = T;}}); //updates board once all threads finish doGeneration
...
class UI extends JPanel {
    ...
    public UI(int N, RootPaneContainer pane, int pauseIterations,
              boolean headless, boolean glider, int num_threads) {

        numThreads = num_threads;
        boardSize = N;
        final UI u = this;
        c = new Coordinator(pauseIterations);
        lb = new LifeBoard(N, c, u, headless, glider, num_threads);
    ...
    public void onRunClick() {
        /* Match threads to rows in the board  */
        int n = boardSize / numThreads;
        int start = 0;
        int end = boardSize - (numThreads * n); //first thread picks up any remainders
        for(int i = 0; i < numThreads; i++){
            end += n;
            Worker w = new Worker(lb, c, this, start, end);
            w.start(); //invoke run() in worker class
            start += n;
        }
    }
}
