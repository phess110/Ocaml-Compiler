    ...
    def __next_tk(self, limit=None):
        ...
        while not limit or i < limit:
            ...
            if (p.match(c) or p.match(prev_c)) \
                    and not (prev_c == c and c in '&|.<>+-=') \
                    and not (prev_c == '<' and c == '=') \
                    and not (prev_c == '>' and c == '=') \
                    and not (prev_c == '!' and c == '='):
                rem_char = c
                break
...
class Parser:
    def __init__(self, tokenizer, init_prod, grammar, debug=None):
        """
        ...
        while stack:
            ...
            if isinstance(prod, str):
                ...
                # terminal
                    ...
                    if prod == 'number':
                        ...
                        if tk == None or not isinstance(tk, int):
                            line, char = self.tokenizer.position()
                            msg = f'Unexpected token "{tk}" ' if tk else ''
                            raise ParseError(f'{msg}Expected an integer at line {line}, char {char}')
                        nodes.append(tk)
                ...
                if isinstance(two, int):
                    ...
                    c = (comb_name, [nodes[len(nodes)-amt_skip + i] for i in range(0, amt_skip)])
